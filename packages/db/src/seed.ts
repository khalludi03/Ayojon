...
async function seedProducts(vendorIds: Array<string>) {
  ...
  for (let i = 0; i < SAMPLE_PRODUCTS.length; i++) {
    ...
    if (existing.length > 0) {
      ...
      await db
        .delete(schema.productShippingOptions)
        .where(eq(schema.productShippingOptions.productId, productId))
      await db
  ...
  console.log(
    `  ✓ Upserted ${upserted} products with images, variants, event types, and shipping`,
  )
}
...
async function seedSampleUsers() {
  ...
  console.log(
    `  ✓ Now have ${userIds.length} users (ensured ${usersNeeded} new)`,
  )
  return userIds
}
...
async function seedReviews(userIds: Array<string>) {
  ...
  console.log(
    `  ✓ Ensured reviews for ${products.length} products (${upserted} new reviews added)`,
  )
}
...
async function main() {
  ...
  try {
    ...
    const expectedSubs = CATEGORIES_DATA.reduce(
