...
export const adminRouter = os.router({
  listUsers: adminProcedure
    .route({
      ...
      summary: 'List Users',
      description:
        'Lists users with search, pagination, role and vendor status filtering.',
      tags: ['Admin'],
    })
    .input(
      z.object({
    ...
    .handler(async ({ input }) => {
      ...
      if (input.vendorStatus)
        conditions.push(eq(user.vendorStatus, input.vendorStatus))
      if (input.search) {
        conditions.push(
          or(
  ...
  listVendors: adminProcedure
    .route({
      ...
      description:
        'Lists vendors with search, pagination and status filtering.',
      tags: ['Admin'],
    })
    .input(
      z.object({
    ...
    .handler(async ({ input }) => {
      ...
      const vendorList = await db
        .select({
          ...
          productCount:
            sql<number>`(SELECT count(*) FROM ${products} WHERE ${products.vendorId} = ${vendors.id})`.mapWith(
              Number,
            ),
    ...
    .handler(async ({ input }) => {
      return await db.transaction(async (tx) => {
        ...
        if (input.isActive !== undefined) {
          ...
          console.log(
            `[Admin Update Vendor] Synced user vendorStatus to "${newVendorStatus}" for vendor ${vendor.name}`,
          )
        }
  ...
  updateVendorApplicationStatus: adminProcedure
    .route({
      ...
      description:
        'Updates vendor application status (approve, reject, suspend).',
      tags: ['Admin'],
    })
    .input(
      z.object({
    ...
    .handler(async ({ input }) => {
      try {
        console.log(
          `[Admin Action] Processing vendor status update to ${input.vendorStatus} for user ${input.userId}`,
        )
        const result = await db.transaction(async (tx) => {
          ...
          if (updatedUsers.length === 0) {
            console.error(
              `[Admin Action] User ${input.userId} not found during status update`,
            )
            throw new ORPCError('NOT_FOUND', { message: 'User not found' })
          }
          ...
          if (input.vendorStatus === 'approved') {
            console.log(
              `[Admin Action] Application approved, looking up application details for user ${input.userId}`,
            )
            ...
            if (applications.length > 0) {
              ...
              console.log(
                `[Admin Action] Found application ${app.id}, promoting to vendor`,
              )
              ...
              console.log(
                `[Admin Action] Inserting/Updating vendor record with slug ${vendorSlug}`,
              )
              await tx
                .insert(vendors)
                .values({
              ...
              console.warn(
                `[Admin Action] No application found for user ${input.userId}, but status was set to approved.`,
              )
        ...
        throw new ORPCError('INTERNAL_SERVER_ERROR', {
          message:
            error instanceof Error
              ? error.message
              : 'Failed to update vendor application status',
        })
      }
    }),
  ...
  getVendorApplicationDetails: adminProcedure
    .route({
      ...
      description:
        'Get full details of a vendor application including documents.',
      tags: ['Admin'],
    })
    .input(
      z.object({
  ...
  deleteVendor: adminProcedure
    .route({
      ...
      description:
        'Deletes a vendor profile permanently. User account remains but role is reverted.',
      tags: ['Admin'],
    })
    .input(
      z.object({
    ...
    .handler(async ({ input, context }) => {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        console.log(
          `[Admin Delete Vendor] Deleted vendor ${vendor.name} (${input.id}), removed ${deletedFiles.length} files from S3`,
        )
  ...
  listAllProducts: adminProcedure
    .route({
      ...
      description:
        'Lists all products across the platform with advanced filtering.',
      tags: ['Admin'],
    })
    .input(
      z.object({
    ...
    .handler(async ({ input }) => {
      ...
      if (parsedInput.vendorId)
        conditions.push(eq(products.vendorId, parsedInput.vendorId))
      if (parsedInput.categoryId)
        conditions.push(eq(products.categoryId, parsedInput.categoryId))
      if (parsedInput.search) {
        conditions.push(
          or(
  ...
  updateProductPromotions: adminProcedure
    .route({
      ...
      description:
        'Sets featured, hot deal, or flash deal flags for a product.',
      tags: ['Admin', 'Homepage'],
    })
    .input(
      z.object({
        ...
        dealType: z
          .enum(['flash', 'hot', 'daily', 'clearance', 'bundle'])
          .nullable()
          .optional(),
      }),
    )
  ...
  adminDeleteProduct: adminProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      return await db.transaction(async (tx) => {
        ...
        console.log(
          `[Admin Notice] Product ${input.id} removed. Reason: ${input.reason}. Deleted ${deletedFiles.length} images from S3.`,
        )
  ...
  listOrders: adminProcedure
    .route({
      ...
      description:
        'Lists all orders across the platform with search and pagination.',
      tags: ['Admin'],
    })
    .input(
      z.object({
  ...
  listPendingPayments: adminProcedure
    ...
    .handler(async ({ input }) => {
      const paymentsList = await paymentService.getPendingPayments(
        input.limit,
        input.offset,
      )
      return {
  ...
  verifyPayment: adminProcedure
    .route({
      ...
      description:
        'Admin verifies a bKash payment and moves order to payment_received status.',
      tags: ['Admin', 'Payments'],
    })
    .input(
      z.object({
  ...
  rejectPayment: adminProcedure
    .route({
      ...
      description:
        'Admin rejects a bKash payment with reason and moves order back to awaiting_payment.',
      tags: ['Admin', 'Payments'],
    })
    .input(
      z.object({
  ...
  processPayout: adminProcedure
    .route({
      ...
      description:
        'Admin processes payment to vendor and records transaction details.',
      tags: ['Admin', 'Payouts'],
    })
    .input(
      z.object({
  ...
  markPayoutFailed: adminProcedure
    ...
    .input(
      z.object({
        ...
        reason: z
          .string()
          .min(10, 'Failure reason must be at least 10 characters'),
      }),
    )
    .handler(async ({ input, context }) => {
      const adminId = context.session.user.id
      const result = await payoutService.markPayoutFailed(
        input.payoutId,
        adminId,
        input.reason,
      )
  ...
  createPayoutForOrder: adminProcedure
    .route({
      ...
      description:
        'Manually create payout records for an order that has been completed.',
      tags: ['Admin', 'Payouts'],
    })
    .input(
      z.object({
  ...
  listAllBanners: adminProcedure
    .route({
      ...
      description:
        'Lists all homepage banners (active and inactive) for admin management.',
      tags: ['Admin', 'Homepage'],
    })
    .handler(async () => {
  ...
  listAllPromoCards: adminProcedure
    .route({
      ...
      description:
        'Lists all 4 homepage promotional cards for admin management.',
      tags: ['Admin', 'Homepage'],
    })
    .handler(async () => {
