  ...
  return await db
    .transaction(async (tx) => {
      ...
      await tx.insert(payments).values({
        ...
        status: orderData.paymentDetails?.transactionId
          ? 'submitted'
          : 'pending',
...
export async function getVendorOrders(
  ...
  return ordersData.filter((order) => orderIds.includes(order.id))
}
...
export async function vendorOwnsOrderItems(
  ...
  const items = await db
    ...
    .from(orderItems)
    .where(
      and(eq(orderItems.orderId, orderId), eq(orderItems.vendorId, vendorId)),
    )
    .limit(1)
