...
import { eq, avg, sql, and, inArray, countDistinct } from "drizzle-orm";
...
export async function updateVendorScore(vendorId: string, tx: any = db) {
  try {
    ...
    console.log(`[updateVendorScore] Updated vendor ${vendorId}: AvgRating=${avgProductRating}, CompletionRate=${completionRate.toFixed(2)}, Score=${score.toFixed(2)}`);
  } catch (error) {
