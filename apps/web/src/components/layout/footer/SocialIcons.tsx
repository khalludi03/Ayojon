// Professional Social Media Icons with official brand colors
// Using custom SVG icons for authentic brand representation
...
function XIcon({ className }: SocialIconProps) {
  return (
    <svg viewBox="0 0 24 24" className={className} aria-label="X (Twitter)">
      <path
        fill="currentColor"
...
export function SocialIcons() {
  return (
    <div className="flex items-center gap-4">
      <span className="text-sm font-medium text-[hsl(var(--foreground))]">Follow Us:</span>
      <div className="flex gap-2">
        {socialLinks.map((social) => (
          <a
            ...
            className={`group flex h-10 w-10 items-center justify-center rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--card))] shadow-sm transition-all duration-300 hover:scale-110 hover:shadow-lg hover:border-transparent ${social.bgColor}`}
            aria-label={`Follow us on ${social.label}`}
          >
            <social.Icon className={`h-5 w-5 transition-colors duration-300 ${social.iconColor}`} />
          </a>
        ))}
      </div>
    </div>
