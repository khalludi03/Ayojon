...
import { ArrowRight, Heart, Package, ShoppingBag, User } from 'lucide-react'
          ...
          <div className="space-y-3 sm:space-y-4">
            {isLoading ? (
              ...
              Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 rounded-xl border border-[hsl(var(--border))] p-4"
                >
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-2 sm:gap-3 md:gap-4">
                    <div className="flex flex-col items-start sm:items-end">
                      <p className="text-sm font-semibold sm:text-base">
                        {formatCurrencyPrice(parseFloat(order.total as any))}
                      </p>
                      <Badge
                        variant="secondary"
                        className={`${(statusConfig[order.status] ?? { color: 'bg-slate-500' }).color} mt-1 text-[10px] text-white sm:text-xs`}
                      >
                        {
                          (
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-8 px-2 font-bold text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 gap-1 hidden sm:flex"
                    >
                      View Details
            ...
            {recentOrders.length > 0 ? (
              ...
              >
                <Link
                  ...
                  params={{ orderNumber: latestOrder.orderNumber }}
                >
