  ...
  useEffect(() => {
    if (session?.user && !isStep1Skipped) {
      ...
      if (currentStep === 1) {
        ...
        toast.info(
          `Logged in as ${session.user.email}. Skipping account creation.`,
        )
      }
    }
    ...
    try {
      ...
      if (formData.storeBanner instanceof File) {
        bannerUrl = await uploadFile(
          ...
          'vendor/banners',
          formData.storeBanner.type,
        )
      }
      if (formData.tradeLicense instanceof File) {
        tradeLicenseUrl = await uploadFile(
          formData.tradeLicense,
          'vendor/documents',
          formData.tradeLicense.type,
        )
      }
      if (formData.identification instanceof File) {
        identificationUrl = await uploadFile(
      ...
      if (formData.bankDetails instanceof File) {
        bankDetailsUrl = await uploadFile(
          formData.bankDetails,
          'vendor/documents',
          formData.bankDetails.type,
        )
      }
      ...
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        ...
        <div className="flex flex-col justify-center gap-4 sm:flex-row">
          ...
          {user?.vendorStatus === 'approved' ? (
            ...
            <Button
              onClick={() => navigate({ to: '/vendor/application-pending' })}
            >
