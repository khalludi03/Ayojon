...
import { Package, MapPin, CreditCard, Truck, Check, Loader2 } from 'lucide-react';
...
export function OrderDetailView({ order: initialOrder }: OrderDetailViewProps) {
  ...
  const updateStatus = orpc.vendor.updateOrderStatus.useMutation({
    onSuccess: (updatedOrder: any) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['vendor', 'getOrderDetails'] });
    },
    onError: (err: any) => {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-[hsl(var(--card))] border-b border-[hsl(var(--border))] p-6">
