  ...
  if (active && payload && payload.length) {
    return (
      <div className="rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-3 shadow-xl">
        <p className="text-[10px] font-bold text-[hsl(var(--muted-foreground))] uppercase tracking-widest mb-1">
          {label}
        </p>
    ...
    <div className="rounded-xl sm:rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4 sm:p-6 md:p-8 shadow-sm transition-all hover:shadow-md">
      <div className="mb-4 sm:mb-6 md:mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 md:gap-6">
        ...
        <div className="flex items-center gap-1 sm:gap-2 bg-[hsl(var(--muted))]/50 p-0.5 sm:p-1 rounded-lg sm:rounded-xl border border-[hsl(var(--border))]">
          <button className="px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-md sm:rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-colors">
            14D
          </button>
          <button className="px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-md sm:rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-wider bg-[hsl(var(--card))] text-[hsl(var(--primary))] shadow-sm border border-[hsl(var(--border))]">
            30D
          </button>
        </div>
      </div>
      ...
      <div className="mb-4 sm:mb-6 md:mb-10 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
        <div className="space-y-0.5 sm:space-y-1">
          <p className="text-[9px] sm:text-[10px] font-bold text-[hsl(var(--muted-foreground))] uppercase tracking-widest">
            Total Gross Revenue
          </p>
          <div className="flex items-baseline gap-1 sm:gap-2">
            <span className="text-xl sm:text-2xl md:text-3xl font-black text-[hsl(var(--foreground))] tracking-tighter">
        ...
        <div className="space-y-0.5 sm:space-y-1 sm:border-l sm:border-[hsl(var(--border))] sm:pl-4 md:pl-8">
          ...
          <div className="flex items-baseline gap-1 sm:gap-2">
            <>
              <span className="text-xl sm:text-2xl md:text-3xl font-black text-[hsl(var(--foreground))] tracking-tighter">
                ৳
                {Math.floor(totalRevenue / (data.length || 1)).toLocaleString()}
              </span>
              <span className="text-[10px] sm:text-xs font-bold text-[hsl(var(--muted-foreground))]">
                / day
              </span>
            </>
          </div>
