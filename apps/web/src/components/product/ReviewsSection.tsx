...
export function ReviewsSection({ productId, productName, productImage }: ReviewsSectionProps) {
  ...
  const { data: rawReviews, isLoading } = useQuery(
    (orpc.review.getProductReviews as any).queryOptions({
  ...
  const reviews = useMemo(() => {
    ...
    return filtered.map(r => ({
      ...
      user: {
        ...
        avatar: (r.user as any).image || undefined,
      },
      images: (r.images || []).map((img: any) => ({ url: img.url, alt: img.alt || '' })),
      helpfulVotes: (r as any).helpfulVotes || 0,
      notHelpfulVotes: (r as any).notHelpfulVotes || 0,
    })) as Review[];
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between mb-8">
          ...
          {(canReviewData as any)?.canReview && (
            <Button onClick={() => setIsModalOpen(true)} className="gap-2 bg-indigo-600 hover:bg-indigo-700 font-bold">
                  ...
                  {sortOptions.map((option) => (
                    <button
                      ...
                      className={cn(
                        ...
                        sort === option.value && 'bg-slate-50 dark:bg-slate-900 font-bold text-indigo-600'
                      )}
                      onClick={() => {
