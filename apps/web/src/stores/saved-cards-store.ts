...
const readSavedCards = (): Array<SavedCard> => {
  ...
  try {
    ...
    const parsed = JSON.parse(stored) as Array<SavedCard>
...
const writeSavedCards = (cards: Array<SavedCard>) => {
  if (typeof window === 'undefined') {
    return
  }
...
export const getSavedCards = (): Array<SavedCard> => readSavedCards()
