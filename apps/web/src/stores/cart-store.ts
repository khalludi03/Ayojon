  ...
  const syncWithBackend = async () => {
    ...
    try {
      ...
      if (allLocalItems.length > 0) {
        ...
        const transformedItems: Array<CartItem> = (backendItems as Array<any>)
          .filter((i) => i.savedForLater === 0)
          .map((item) => ({
            id: generateId(),
            productId: item.productId,
        ...
        const transformedSaved: Array<CartItem> = (backendItems as Array<any>)
          .filter((i) => i.savedForLater === 1)
          .map((item) => ({
            id: generateId(),
            productId: item.productId,
            product: item.product,
