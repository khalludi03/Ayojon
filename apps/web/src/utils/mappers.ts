import type { Product, DealProduct, ProductImage, ProductBadge } from '@/types';
...
export function mapDbProductToFrontend(dbProduct: any): Product {
  ...
  const images: ProductImage[] = (dbProduct.images || []).map((img: any) => ({
    url: img.url,
  ...
  return {
    ...
    badges: (content.badges as ProductBadge[]) || [],
...
export function mapDbProductToDealFrontend(dbProduct: any): DealProduct {
  ...
  return {
    ...
    dealEndsAt: dbProduct.dealEndsAt?.toString() || new Date(Date.now() + 86400000).toISOString(),
    dealStartedAt: dbProduct.dealStartsAt?.toString() || dbProduct.createdAt?.toString() || new Date().toISOString(),
  };
}
