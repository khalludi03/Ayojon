...
export const generateInvoicePDF = (data: InvoiceData) => {
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { cellWidth: 12, halign: 'center', fontStyle: 'bold', textColor: lightGray },
      1: { cellWidth: 'auto' },
      2: { cellWidth: 18, halign: 'center' },
      3: { cellWidth: 35, halign: 'right' },
      4: { cellWidth: 35, halign: 'right', fontStyle: 'bold', textColor: darkGray }
    },
    alternateRowStyles: {
      fillColor: bgLight
    }
  ...
  doc.setFillColor(...bgLight);
  doc.roundedRect(summaryBoxX, summaryY, 65, Number(data.discount) > 0 ? 42 : 36, 2, 2, 'F');
  ...
  doc.text(formatCurrency(data.subtotal), summaryBoxX + 60, lineY, { align: 'right' });
  lineY += 6;

  doc.text('Shipping:', summaryBoxX + 5, lineY);
  doc.text(formatCurrency(data.shippingCost), summaryBoxX + 60, lineY, { align: 'right' });
  lineY += 6;

  doc.text('Tax & VAT:', summaryBoxX + 5, lineY);
  doc.text(formatCurrency(data.tax), summaryBoxX + 60, lineY, { align: 'right' });
  lineY += 6;

  if (Number(data.discount) > 0) {
    doc.setTextColor(...secondaryColor);
    doc.text('Discount:', summaryBoxX + 5, lineY);
    doc.text(`-${formatCurrency(data.discount)}`, summaryBoxX + 60, lineY, { align: 'right' });
    doc.setTextColor(...darkGray);
    lineY += 6;
  }
  ...
  doc.setFontSize(12);
  doc.text(formatCurrency(data.total), summaryBoxX + 60, lineY + 2, { align: 'right' });
  ...
  doc.setTextColor(...primaryColor);
  doc.text('Thank you for your business!', pageWidth / 2, footerY + 27, { align: 'center' });
