...
function AdminOrdersPage() {
  ...
  const { data, isLoading } = useQuery(
    orpc.admin.listOrders.queryOptions({
      input: {
  ...
  const updateStatusMutation = useMutation(
    orpc.admin.updateOrderStatus.mutationOptions({
      onSuccess: () => {
          ...
          <Button
            ...
            onClick={() =>
              queryClient.invalidateQueries({
                queryKey: orpc.admin.listOrders.key(),
              })
            }
            className="gap-2"
          >
        ...
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-xl shadow-slate-200/50 dark:shadow-none">
          <div className="flex flex-col sm:flex-row gap-3">
            <form onSubmit={handleSearch} className="flex gap-3 flex-1">
              ...
              <Button
                type="submit"
                className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold h-12 px-6 rounded-xl shadow-lg shadow-indigo-500/30 transition-all hover:shadow-xl hover:shadow-indigo-500/40"
              >
                ...
                {!data ? (
                  ...
                  data.orders.map((order) => {
                    ...
                    return (
                      <tr
                        key={order.id}
                        className="group hover:bg-gradient-to-r hover:from-indigo-50/50 hover:to-purple-50/30 dark:hover:from-indigo-950/20 dark:hover:to-purple-950/10 transition-all duration-200"
                      >
                          ...
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-9 w-9 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
                              >
                            ...
                            >
                              ...
                              {order.status === 'payment_submitted' && (
                                <>
                                  <DropdownMenuItem
                                    onClick={() =>
                                      updateStatus(order.id, 'payment_received')
                                    }
                                  >
                                    <CheckCircle className="mr-2 h-4 w-4 text-green-600" />
                                    <span className="text-green-600 font-bold">
                                  ...
                                  <DropdownMenuItem
                                    onClick={() =>
                                      updateStatus(order.id, 'payment_rejected')
                                    }
                                  >
