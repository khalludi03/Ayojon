...
function AdminProductsPage() {
  ...
  const { data, isLoading } = useQuery(
    orpc.admin.listAllProducts.queryOptions({
      input: {
  ...
  const { data: vendorsData } = useQuery(
    orpc.admin.listVendors.queryOptions({ input: { limit: 100 } }),
  )

  const promotionsMutation = useMutation(
    orpc.admin.updateProductPromotions.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: orpc.admin.listAllProducts.key(),
        })
  ...
  const removeMutation = useMutation(
    orpc.admin.adminDeleteProduct.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: orpc.admin.listAllProducts.key(),
        })
  ...
  const toggleDealType = (
    id: string,
    currentDealType: string | null | undefined,
    nextType: 'flash' | 'hot',
  ) => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium mt-1">
              Monitoring {data?.totalCount ?? 0} active listings across the
              platform.
            </p>
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="sound-lighting">
                    Sound & Lighting
                  </SelectItem>
                  <SelectItem value="furniture-tents">
                    Furniture & Tents
                  </SelectItem>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            variant={
                              product.dealType === 'hot' ? 'default' : 'outline'
                            }
                            onClick={() =>
                              toggleDealType(
                                product.id,
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                openRemoveDialog(product.id, product.title)
                              }
      ...
      <Dialog open={isRemoveDialogOpen} onOpenChange={setIsRemoveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              You are removing <strong>{productToRemove?.title}</strong> from
              the marketplace. This will notify the vendor and hide the product
              from all customers.
            </DialogDescription>
          </DialogHeader>
