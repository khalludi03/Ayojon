...
function AdminUsersPage() {
  ...
  const updateMutation = useMutation(orpc.admin.updateUser.mutationOptions({
    onSuccess: () => {
      ...
      if (selectedUser) {
        queryClient.invalidateQueries({ queryKey: orpc.admin.getUserDetails.key({ id: selectedUser }) });
      }
  ...
  const deleteMutation = useMutation(orpc.admin.deleteUser.mutationOptions({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: orpc.admin.listUsers.key() });
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8">
                  ...
                  <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={cn(
                        ...
                        <span className={cn("mr-1.5 h-1.5 w-1.5 rounded-full", user.isDeactivated ? "bg-red-600" : "bg-green-600")} />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          ...
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => changeRole(user.id, user.role === 'vendor' ? 'customer' : 'vendor')}>
                            <UserCog className="mr-2 h-4 w-4" /> 
                            Change to {user.role === 'vendor' ? 'Customer' : 'Vendor'}
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => toggleSuspension(user.id, user.isDeactivated)}>
                            {user.isDeactivated ? (
                              <><UserCheck className="mr-2 h-4 w-4" /> Activate Account</>
                            ) : (
                              <><UserX className="mr-2 h-4 w-4 text-red-600" /> <span className="text-red-600">Suspend Account</span></>
                            )}
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem 
      ...
      <Dialog open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black tracking-tight">User Details</DialogTitle>
            <DialogDescription>Full account information and statistics.</DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                  ...
                  <p className="text-sm font-mono font-medium truncate">{userDetails.user.id}</p>
                </div>
                <div className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                  <div className="flex items-center gap-2 text-indigo-600 mb-1">
                  ...
                  <p className="text-sm font-bold">{new Date(userDetails.user.createdAt).toLocaleDateString('en-US', { dateStyle: 'long' })}</p>
                </div>
                <div className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                  <div className="flex items-center gap-2 text-indigo-600 mb-1">
                  ...
                  <p className="text-2xl font-black">{userDetails.stats.totalOrders}</p>
                </div>
                <div className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 space-y-1">
                  <div className="flex items-center gap-2 text-indigo-600 mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={cn(
                      ...
                      userDetails.user.isDeactivated ? "bg-red-600" : "bg-green-600"
                    )} />
                    <p className="text-sm font-bold">{userDetails.user.isDeactivated ? "Suspended" : "Active"}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete the account for <strong>{userToDelete?.name}</strong>? This action cannot be undone and will remove all associated data.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button 
              variant="destructive" 
              onClick={() => userToDelete && deleteMutation.mutate({ id: userToDelete.id })}
