...
function AdminSettingsPage() {
  ...
  const { data: settings, isLoading } = useQuery(
    orpc.admin.getPlatformSettings.queryOptions(),
  )
  ...
  const updateMutation = useMutation(
    orpc.admin.updatePlatformSettings.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: orpc.admin.getPlatformSettings.key(),
        })
          ...
          <Section icon={Zap} title="Campaign & Deals">
            <div className="space-y-6">
              <div className="space-y-2 max-w-sm">
                <Label
                  htmlFor="flashDealEndsAt"
                  className="text-[10px] font-black uppercase tracking-widest text-slate-500"
                >
                  Flash Deal Global End Time
                </Label>
                <div className="relative">
                  <Input
                    id="flashDealEndsAt"
                    ...
                    value={
                      formData.flashDealEndsAt
                        ? new Date(
                            new Date(formData.flashDealEndsAt).getTime() -
                ...
                <p className="text-[10px] text-slate-400 font-medium">
                  This overrides individual product deal times for the homepage
                  countdown.
                </p>
                <Button
                  variant="outline"
          ...
          <Section icon={ShieldAlert} title="Feature Visibility">
            <div className="space-y-6">
              ...
              <ToggleRow
                ...
                onChange={(v) =>
                  handleBooleanChange('enableVendorRegistration', v)
                }
              />
                ...
                <div className="flex items-start justify-between p-4 bg-red-50 dark:bg-red-950/20 rounded-2xl border border-red-100 dark:border-red-900/50">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-red-700/70 dark:text-red-400/70 font-medium">
                      Hides store from public and prevents all orders. Use only
                      for updates.
                    </p>
                  </div>
                  <Checkbox
                    ...
                    onCheckedChange={(v) =>
                      handleBooleanChange('isMaintenanceMode', v)
                    }
