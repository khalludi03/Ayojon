...
export const Route = createFileRoute('/vendor/$vendorId')({
  ...
  loader: async ({ params }) => {
    const vendor = await orpcClient.product.getVendorBySlug({ slug: params.vendorId });
    return { vendor };
  },
  head: ({ loaderData, params }) => {
    ...
    if (!vendor) {
      return {
        meta: [
          ...
          { name: 'description', content: 'The vendor you are looking for could not be found.' },
        ],
      };
    }

    const title = `${vendor.name} - Event Vendor | Ayojon`;
    const description = vendor.description?.slice(0, 160) || `Shop from ${vendor.name}, a trusted vendor on Ayojon. Quality event rentals and supplies.`;
    ...
    return {
      meta: [
        ...
        { name: 'keywords', content: `${vendor.name}, event vendor, event rental, Ayojon vendor` },
